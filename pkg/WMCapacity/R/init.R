...
CovHelp="In order to model the covariance between two sets of parameters, you can add them to Covariance 'groups'. Each covariance group must have the same number of parameters, and contain more than two sets of parameters. It is not recommended to add categorical by categorical interactions in a covariance group, due to the problems with interpreting some such covariances.\n\nEnsure when interpretting any covariance that the order of the parameters makes sense - WOMMBAT may not have put the parameters in the order you expect.",
